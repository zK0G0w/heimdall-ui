...
- MFA 多因素认证（TOTP 绑定/验证、恢复码、管理端策略配置）
...
└── views/             # 页面组件
    ...
    ├── user/          # 个人中心（安全设置、MFA 管理）
    ├── tenant/        # 多租户
    ├── open/          # 能力开放
    ├── schedule/      # 任务调度
    ├── monitor/       # 系统监控
    └── login/         # 登录（含 MFA 验证/绑定组件）
```
...
- **登录二次验证** — 登录后触发 TOTP 6 位动态码验证（分离输入框、shake 动效、恢复码切换）
- **绑定引导** — 被强制要求 MFA 时登录流程自动引导完成绑定
- **个人中心管理** — 安全设置中绑定/解绑/重新生成恢复码
- **管理端策略** — 系统配置全局强制 MFA 开关、角色级强制 MFA 字段
